...
class StockBalanceReport:
	...
	def run(self):
		...
		self.add_additional_store_uom_columns()
	...
	def add_additional_store_uom_columns(self):
		if not self.filters.get("include_store_uom"):
			return

		conversion_factors = self.get_itemwise_conversion_factor_for_store_uom()
		make_additional_store_uom_columns(self.columns, self.data, self.filters.include_uom, conversion_factors)
	...
	def get_itemwise_conversion_factor_for_store_uom(self):
		query = """select
				itm.name as item_code,
				itm.store_uom,
				uom_cd.conversion_factor
				from `tabItem` as itm
				left join `tabUOM Conversion Detail` as uom_cd on itm.name = uom_cd.parent and itm.store_uom = uom_cd.uom
				where itm.disabled=0 and itm.store_uom is not null
			"""
		if self.filters.item_code:
			query += " and itm.name='{0}'".format(self.filters.item_code)

		result = frappe.db.sql(query, as_dict=True)
		return {d.item_code: d.conversion_factor or 1 for d in result}
...
def make_additional_store_uom_columns(columns, result, include_uom, conversion_factors):
	...
	if include_uom:
		for col_idx in list(reversed(range(0, len(columns)))):
			...
			if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
				next_col = col_idx
				if "_alt" not in columns[next_col]["fieldname"]:
					columns.insert(next_col+1, col.copy())
					columns[next_col+1]["fieldname"] += "_alt_store_uom"
					convertible_column_map[col.get("fieldname")] = frappe._dict(
						{"converted_col": columns[next_col+1]["fieldname"], "for_type": col.get("convertible")}
					)
					if col.get("convertible") == "rate":
						columns[next_col+1]["label"] += f" (per Store UOM)"
					else:
						columns[next_col+1]["label"] += f" (Store UOM)"
	else:
		for col_idx in list(reversed(range(0, len(columns)))):
			col = columns[col_idx]
			if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
				...
				columns[next_col]["fieldname"] += "_alt_store_uom"
				convertible_column_map[col.get("fieldname")] = frappe._dict(
					{"converted_col": columns[next_col]["fieldname"], "for_type": col.get("convertible")}
				)
				if col.get("convertible") == "rate":
					columns[next_col]["label"] += f" (per Store UOM)"
				else:
					columns[next_col]["label"] += f" (Store UOM)"
